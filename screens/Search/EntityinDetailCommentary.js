...
class EntityinDetailCommentary extends Component {
        ...
        >
          <View style={{ paddingBottom: 20 }}>
            <View style={{ flexDirection: "column" }}>
              <View style={{ justifyContent: "center", alignItems: "center" }}>{this.state.title}</View>
          ...
          <View style={{ paddingTop: 5 }}>{this.state.text}</View>
  ...
  GoCallAPI() {
    var urlEntityinDetailCommentary =
    ...
    fetch(urlEntityinDetailCommentary)
      .then(response => response.json())
      .then(responseJson => {

        let list_comment = responseJson.data.list_comment
        this.handleChangeTab(
          DCT.getValue("commentaryabout", this.language) + " " + this.keyplain
        );
    
        let mytext = list_comment[0].text;
        mytext = this.MyParser.DoParserDiscovery(mytext, this.props.STORE_BIBLE.IS_SHOW_DARKMODE);
        if (this._isMounted) {
          this.setState(
            {
              text: mytext,
  ...
  GoCallAPI2() {
    if (this._isMounted) {
      this.key = this.MyParser.DoParserDiscovery(this.key, this.props.STORE_BIBLE.IS_SHOW_DARKMODE);
      this.setState(
        {
          vref: this.vref,
          title: this.key,
