...
class EntityinBible extends Component {
  ...
  componentDidMount = () => {
    ...
    this.handleChangeTab(
      this.keyword + " " + DCT.getValue("inthebible", this.language)
    );
  ...
  render() {
    if (!this.state.isLoading) {
      ...
      if (this.entityinbible > 0 || this.ok) {
        return (
              ...
              renderItem={({ item, index }) => (
                ...
                >
                  <View style={{ flexDirection: "row" }}>
                    <TouchableOpacity
                      onPress={() => {
                        this.EntityinBibleDetail(item.verse_id, this.language);
                      }}
                    >
                      <Text
                        style={{
    ...
    this.setState({ fetching_from_server: true }, () => {
      ...
      this.timer = setTimeout(() => {
        ...
        var urlEntityinBible =
          ...
          "&skip=" +
          this.skip +
          "&fragment_size=4000&type=B&search_in_desc=" +
          this.isindefinition +
          "&search_fuzzy=" +
          this.isfuzzy +
          "&lang_code=" +
  ...
  GoCallAPI() {
    ...
    fetch(urlEntityinBible)
      ...
      .then(responseJson => {
        ...
        let list_search_data = JSON.parse(this.list_search);
        this.entityinbible = list_search_data.length;
        if (this.entityinbible > 0) this.ok = true;
        this.setState({
          serverData: [...this.state.serverData, ...list_search_data],
  ...
  EntityinBibleDetail(verse_id, language) { }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(EntityinBible);
