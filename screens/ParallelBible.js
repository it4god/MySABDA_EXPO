...
class ParalelBible extends Component {
  ...
  componentDidMount = () => {
    ...
    this.handleChangeTab(DCT.getValue("parelleltext", this.language));
  ...
  setBibleVersion(value) {
    this.parallelbible = value;
    this._storeData();
    this.props.ACT_SetBibleParallel(value);
    this.props.ACT_SetParallel("true");
  ...
  _storeData = async () => {
    try {
      await AsyncStorage.setItem("bibleparallel", this.parallelbible);
    } catch (error) {
      console.log(error);
    }
    try {
      await AsyncStorage.setItem("parallel", "true");
    } catch (error) {
      console.log(error);
    }
  };
...
export default connect(mapStateToProps, mapDispatchToProps)(ParalelBible);
