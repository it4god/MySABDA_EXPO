...
export default class TagParser extends Component {
  constructor(props, darkmode) {
    ...
    this.textclick = [];
    this.is_show_dark_mode = darkmode;
  ...
  DoParserBibleFullVersion(
    ...
    this.rendercontent = "";
    if (rendercontent !== null) this.rendercontent = rendercontent;

    //console.log(this.rendercontent);
    if (this.rendercontent == "rendercontent1") this.textclick = [];
    ...
    this.is_show_dark_mode = is_show_dark_mode;

    if (this.is_show_dark_mode != null) {
      if (this.is_show_dark_mode !== true) {
        this.BACKGROUND_COLOR = "#FFFFFF";
        this.TEXT_COLOR = "#353535";
        this.TEXT_COLOR_URL = "#105B8E";
        this.TEXT_COLOR_JESUS = "#BE4228";
      } else if (this.is_show_dark_mode === true) {
  ...
  DoParserDiscovery(text, is_show_dark_mode, param_parent) {
    text = text.replace(/undefined/g, "");
    //text = text.replace(/verse/g, "chapverse");
    text = text.replace(/<br\/>/g, "\n");
    text = text.replace(/<\/br>/g, "\n");
  ...
  DoParserWord(text, is_show_dark_mode, param_parent) {
    text = text.replace(/verse/g, "chapverse");
    text = text.replace(/<br\/>/g, "\n");
    text = text.replace(/<\/br>/g, "\n");
    text = text.replace(/<\/br>/g, "\n");
  ...
  RenderNodeValuePericope(value) {
    ...
    return (
      <Text
        key={id}
        style={{ fontFamily: "NotoSans-Bold", fontSize: fsize, paddingTop: 0 }}
      >
        {value}
      </Text>
    );
  ...
  RenderNodeValueVerseNumber(value) {
    ...
    return (
      <Text
        style={{ color: this.TEXT_COLOR_URL, fontWeight: "normal" }}
        key={id}
      >
        {" "}
        {value}{" "}
  ...
  RenderNodeValuePoetry(value) {
    ...
    value = value.replace(/seterip/g, "-");
    return (
      <Text key={id} style={{ color: this.TEXT_COLOR, fontStyle: "italic" }}>
        {value}
      </Text>
    );
  }
  ...
  RenderTagDiscovery(tag, objChild, param) {
    ...
    switch (tag) {
      case "addbackground":
        <Text key={id} style={{ flexWrap: "wrap", backgroundColor: "#FFCB6B" }}>
          {value}
        </Text>;
        break;
      case "var":
  ...
  RenderTagBiblex(tag, objChild, param) {
    ...
    switch (tag) {
      ...
      case "biblex":
        let textvalue = objChild.textContent;
        textvalue = textvalue.replace(/seterip/g, "-");
        let node_attribute = objChild.attributes[0];
        let node_attributeVerse = objChild.attributes[1];
        let verse_id = "";
        if (
          !COMethods.isObjectEmpty(node_attribute) &&
  ...
  RenderTagBiblexJesus(tag, objChild, param) {
    ...
    switch (tag) {
      ...
      case "biblex":
        ...
        return this.onLink_SetAttributeBiblexJesus(tag, textvalue, verse_id, param_link);
  ...
  clearTextVerseNumber(span_id, is_show_highlight, id) {
    ...
    try {
      this[item_id].setNativeProps({
        style: { color: "#353535", backgroundColor: "#FFCB6B" },
      });
      ...
      setTimeout(() => {
        ...
        //console.log("This color" + this.BACKGROUND_COLOR)
      }, 1500);
    }
    catch (e) {
  ...
  onLink_SetAttributeBiblexJesus(tag, textvalue, verse_id, param) {
    if (!COMethods.isObjectEmpty(param)) {
      let objAttr = {};
      let value = param.value,
        id = param.id;
        ...
        if (this.is_vref_only) {
          ...
          return (
            <Text
              ...
              key={span_id}
              onPress={() => {
                this.clearTextJesus(span_id, this.is_show_highlight, id);
                ClickReaction(classname + "|" + id + "|" + value + "|" + verse_id, this.props);
              }}
              style={{ color: this.TEXT_COLOR_URL, textAlign: "left" }}
            >
              {value}
  ...
  clearTextJesus(span_id, is_show_highlight, id) {
